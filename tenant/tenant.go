...
func RegisterTenant(db *gorm.DB, cfg TenantConfig) error {
	...
	p := &tenantPlugin{cfg: cfg, excludeSet: excludeSet}
	globalTenantPlugin = p
	return db.Use(p)
}

func (p *tenantPlugin) Name() string { return "gorm-plus:tenant" }
...
func (p *tenantPlugin) injectWhere(db *gorm.DB) {
...
func (p *tenantPlugin) injectCreate(db *gorm.DB) {
	if db.Statement == nil || db.Statement.Context == nil {
		return
	}
	if p.shouldSkip(db.Statement.Context, db) {
...
func (p *tenantPlugin) shouldSkip(ctx context.Context, db *gorm.DB) bool {
...
func (p *tenantPlugin) isExcluded(table string) bool {
	if table == "" {
		return true
	}
	p.mu.RLock()
...
func AddExcludeTable(tables ...string) {
	...
	globalTenantPlugin.mu.Lock()
	defer globalTenantPlugin.mu.Unlock()
	for _, t := range tables {
		globalTenantPlugin.excludeSet[strings.ToLower(t)] = struct{}{}
	}
}
...
func RemoveExcludeTable(tables ...string) {
	...
	defer globalTenantPlugin.mu.Unlock()
	for _, t := range tables {
		delete(globalTenantPlugin.excludeSet, strings.ToLower(t))
	}
}
...
func ExcludedTables() []string {
	...
	globalTenantPlugin.mu.RLock()
	defer globalTenantPlugin.mu.RUnlock()
	tables := make([]string, 0, len(globalTenantPlugin.excludeSet))
	for t := range globalTenantPlugin.excludeSet {
...
func DefaultGetTenantID(ctx context.Context) (string, bool) {
