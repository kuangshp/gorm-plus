...
## 简介
本代码生成器基于 gorm 与 gorm-gen，提供从数据库表结构一键生成 Model、Repository、API、VO、DTO、Mapper 的能力，并内置模板系统与路径解析机制，确保在不同工作目录下都能稳定运行。生成器遵循“Model 每次覆盖、其余文件已存在则跳过”的策略，避免覆盖用户自定义代码；同时支持 go-zero API 代码生成联动。
...
- gormplus.go：统一入口，导出 LoadGeneratorConfig 与 Generate
- README.md：快速开始与使用说明
- version.go：版本常量
...
- 类型映射与验证规则
  - getGoType / getGoTypeForApiDto / getGoTypeForVo：数据库类型到 Go 类型映射
  - generateValidateRule：根据字段特性生成校验规则（required、uuid、email、mobile、enum、范围等）
...
- 路径解析
  ...
  - pathToPkg / getLastPathSegment：路径与包名互转，用于 import 与目录组织
- YAML 加载
...
### 类型映射与验证规则
- 类型映射
  - getGoType：通用映射（字符串、整型、浮点、布尔、时间等）
  - getGoTypeForApiDto：API/DTO 特殊映射（decimal/float/double→string；datetime/timestamp/date→int64）
  - getGoTypeForVo：VO 特殊映射（decimal/float/double→string；datetime/timestamp/date→int64）
- 验证规则
  - generateValidateRule：根据字段是否可空、是否主键、字段名关键词（email/mobile）、注释枚举、状态/类型字段范围等生成规则

章节来源
- [generator.go:719-773](file://generator/generator.go#L719-L773)
- [generator.go:287-320](file://generator/generator.go#L287-L320)

### API/VO/DTO/Mapper 模板详解
- API 模板（api_template.txt）
  - 生成 .api 文件，包含请求/响应结构体、分页结构体、服务路由与处理器
  - 支持字段 JSON 标签、可选字段、校验规则
- DTO 模板（dto_template.txt）
  - 生成 Create/Modify 请求 DTO，支持字段校验
- VO 模板（vo_template.txt）
  - 生成视图对象结构体，排除软删字段
- Repository 模板
  - repository_gen_template.txt：生成 defaultXxxRepository 的完整 CRUD/分页/条件查询实现
  - repository_template.txt：生成用户可扩展的 customerXxxRepository 与接口
...
- [README.md:662-694](file://README.md#L662-L694)
- [gormplus.go:873-897](file://gormplus.go#L873-L897)
